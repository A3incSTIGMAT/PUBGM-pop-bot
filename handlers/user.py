...
async def on_bot_added_to_chat(event: ChatMemberUpdated):
    ...
    if is_admin:
        welcome_text = (
            ...
            f"• `/setlogchannel` — лог-канал действий\n\n"
            f"💰 **Экономика:**\n"
        ...
        welcome_text = (
            f"🤖 **NEXUS** добавлен в чат!\n\n"
            f"⚠️ **У меня нет прав администратора** — доступен только базовый функционал:\n\n"
            f"💰 **Экономика:**\n"
            f"• `/balance` — баланс NCoin\n"
            f"• `/daily` — ежедневный бонус\n"
            f"• `/gift` — дарить монеты\n"
            ...
            f"• 🔐 Капча при входе\n"
            f"• 📊 Статистика активности\n\n"
            f"🚀 **Для получения полного функционала:**\n"
            f"1️⃣ Назначьте меня **администратором чата**\n"
            f"2️⃣ Включите права:\n"
            f"   • `can_restrict_members` — для банов и мутов\n"
            f"   • `can_manage_chat` — для лог-канала и приветствий\n\n"
            f"🔧 После назначения администратором станут доступны:\n"
            f"• `/all` — отметить всех\n"
            f"• `/ban` — забанить нарушителя\n"
            f"• `/mute` — заглушить на время\n"
            f"• `/setwelcome` — настроить приветствие\n"
            f"• `/setlogchannel` — лог-канал действий\n"
            f"• `/setup` — мастер настройки\n\n"
            f"📖 **Справка:** `/help`"
        )
