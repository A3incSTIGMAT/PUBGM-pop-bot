...
# ВЕРСИЯ: 1.0.1-production
# ОПИСАНИЕ: Система рангов — 36 уровней, ИСПРАВЛЕН commit=True
# ============================================
...
XP_ACTIONS = {"message":1,"voice":2,"photo":2,"video":3,"sticker":1,"gif":1,"game_win":50,"game_loss":10,"daily":5,"referral":100}

TIER_REWARDS = {
    "silver":{"coins":500},"gold":{"coins":1000,"vip_days":3},"platinum":{"coins":2000,"vip_days":7},
    "ruby":{"coins":3000},"sapphire":{"coins":5000,"vip_days":14},"emerald":{"coins":7500},
    "diamond":{"coins":10000,"vip_days":30},"brilliant":{"coins":25000,"vip_days":90},
}
...
def get_rank_by_xp(xp: int) -> Tuple[Dict[str, Any], int, int]:
    current_rank, next_rank = RANKS[0], RANKS[1] if len(RANKS) > 1 else None
    for i, rank in enumerate(RANKS):
        if xp >= rank["xp_required"]: current_rank = rank; next_rank = RANKS[i+1] if i+1 < len(RANKS) else None
        else: break
    xp_to_next = next_rank["xp_required"] - xp if next_rank else 0
    return current_rank, xp, xp_to_next

def calculate_level_progress(xp: int, cur: Dict, nxt: Optional[Dict]) -> float:
    if nxt is None: return 100.0
    rng = nxt["xp_required"] - cur["xp_required"]
    if rng <= 0: return 100.0
    return min(100.0, max(0.0, (xp - cur["xp_required"]) / rng * 100))
    ...
    try:
        await db._execute_with_retry("""
            CREATE TABLE IF NOT EXISTS user_ranks (
                ...
                rank_name TEXT DEFAULT 'Серебро V', tier TEXT DEFAULT 'silver',
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )""")
    except DatabaseError as e: logger.error(f"Ranks table error: {e}")

async def get_user_rank(user_id: int) -> Optional[Dict]:
    if db is None or user_id is None: return None
    try:
        row = await db._execute_with_retry("SELECT * FROM user_ranks WHERE user_id = ?", (user_id,), fetch_one=True)
        if row: return dict(row)
        await db._execute_with_retry("INSERT INTO user_ranks (user_id, xp, level, rank_name, tier) VALUES (?,0,1,'Серебро V','silver')", (user_id,))
        return {"user_id":user_id,"xp":0,"level":1,"rank_name":"Серебро V","tier":"silver"}
    except DatabaseError as e: logger.error(f"Get rank error: {e}"); return None

async def add_xp(user_id: int, amount: int, reason: str = "activity") -> Tuple[Optional[int], Optional[Dict]]:
    if db is None or user_id is None or amount <= 0: return None, None
    try:
        rank_data = await get_user_rank(user_id)
        if not rank_data: return None, None
        old_level, new_xp = rank_data["level"], rank_data["xp"] + amount
        new_rank, _, _ = get_rank_by_xp(new_xp)
        new_level = new_rank["level"]
        await db._execute_with_retry(
            "UPDATE user_ranks SET xp=?, level=?, rank_name=?, tier=?, updated_at=CURRENT_TIMESTAMP WHERE user_id=?",
            (new_xp, new_level, new_rank["name"], new_rank["tier"], user_id))
        if new_level > old_level: return new_level, new_rank
        return None, None
    except DatabaseError as e: logger.error(f"Add XP error: {e}"); return None, None

async def award_tier_reward(user_id: int, tier: str) -> bool:
    if tier not in TIER_REWARDS: return False
    reward = TIER_REWARDS[tier]
    try:
        if "coins" in reward: await db.update_balance(user_id, reward["coins"], f"Награда за ранг: {tier}")
        if "vip_days" in reward:
            vip_until = (datetime.now() + timedelta(days=reward["vip_days"])).isoformat()
            await db._execute_with_retry("UPDATE users SET vip_level=MAX(COALESCE(vip_level,0),1), vip_until=? WHERE user_id=?", (vip_until, user_id))
        return True
    except DatabaseError as e: logger.error(f"Award error: {e}"); return False


# ==================== ОТСЛЕЖИВАНИЕ ====================

async def track_activity(user_id: int, action: str, value: int = 1) -> None:
    if action not in XP_ACTIONS: return
...
async def cmd_rank(message: Message) -> None:
    ...
    rank_data = await get_user_rank(user_id)
    if not rank_data: await message.answer("❌ Ошибка загрузки ранга."); return
    ...
    text = (
        ...
        f"👤 Игрок: {first_name}\n📊 Уровень: <b>{current_rank['level']}/36</b>\n"
        f"✨ XP: <b>{format_number(xp)}</b>\n\n📈 Прогресс:\n[{progress_bar}] {progress:.1f}%\n"
    )
    if next_rank: text += f"⏫ Следующий: {next_rank['icon']} {next_rank['name']} (ещё {format_number(xp_to_next)} XP)"
    else: text += "🏆 <b>МАКСИМАЛЬНЫЙ УРОВЕНЬ!</b>"
...
async def cmd_top_ranks(message: Message) -> None:
    ...
    await init_ranks_table()
    try:
        rows = await db._execute_with_retry("""SELECT ur.user_id, ur.xp, ur.level, ur.rank_name, u.first_name, u.username
            FROM user_ranks ur LEFT JOIN users u ON ur.user_id = u.user_id WHERE ur.xp > 0 ORDER BY ur.xp DESC LIMIT 15""", fetch_all=True)
        if not rows: await message.answer("📊 Пока нет данных о рангах."); return
        
        text = "🏆 <b>ТОП-15 ИГРОКОВ ПО РАНГУ</b>\n\n"
        medals = ["🥇","🥈","🥉"] + [f"{i}." for i in range(4,16)]
        for i, row in enumerate(rows):
            medal = medals[i] if i < len(medals) else f"{i+1}."
            name = safe_html_escape((row["first_name"] or row["username"] or "Игрок")[:20])
            text += f"{medal} {get_rank_by_level(row['level'])['icon']} <b>{name}</b> — {format_number(row['xp'])} XP ({row['rank_name']})\n"
        
        await message.answer(text, parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ НАЗАД", callback_data="back_to_menu")]]))
    except DatabaseError as e: logger.error(f"Top ranks error: {e}"); await message.answer("❌ Ошибка загрузки топа.")
...
async def rank_menu_callback(callback: CallbackQuery) -> None:
    ...
    await cmd_rank(callback.message); await callback.answer()

@router.callback_query(F.data == "top_ranks")
async def top_ranks_callback(callback: CallbackQuery) -> None:
    if not callback or not callback.message: return
    await cmd_top_ranks(callback.message); await callback.answer()


# ==================== ИНТЕГРАЦИЯ ====================

async def track_message_activity(user_id: int, message: Message) -> None:
    if message.voice: await track_activity(user_id, "voice")
    elif message.video: await track_activity(user_id, "video")
    elif message.photo: await track_activity(user_id, "photo")
    elif message.sticker: await track_activity(user_id, "sticker")
    elif message.animation: await track_activity(user_id, "gif")
    elif message.text: await track_activity(user_id, "message")
