...
# ВЕРСИЯ: 3.0.1-production
# ОПИСАНИЕ: VIP статусы, покупка, достижения
# ИСПРАВЛЕНИЯ: Убран несуществующий параметр commit
# ============================================
...
async def get_or_create_user(user_id: int, username: Optional[str] = None, first_name: Optional[str] = None) -> Optional[Dict]:
...
async def get_user_xo_stats(user_id: int) -> Tuple[int, int, int]:
    try:
        ...
        if stats: return (stats.get('wins',0) or 0, stats.get('losses',0) or 0, stats.get('games_played',0) or 0)
    except DatabaseError as e: logger.error(f"XO stats error: {e}")
    return 0, 0, 0
...
async def update_user_vip(user_id: int, vip_level: int, days: int = VIP_DURATION_DAYS) -> bool:
    ...
    try:
        ...
        logger.error(f"Failed to update VIP: {e}")
        return False
...
async def check_and_award_achievement_vip(user_id: int, wins: int) -> Optional[int]:
    ...
    for level in sorted(VIP_NAMES.keys()):
        if wins >= VIP_NAMES[level]['wins_required']: vip_level = level
    ...
    if vip_level > 0:
        try:
            ...
            if vip_level > current_vip:
                ...
                return vip_level
        except DatabaseError as e: logger.error(f"Failed to award VIP: {e}")
    return None
...
def format_vip_catalog(balance: int, wins: int, losses: int, games: int) -> str:
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Баланс: {balance} NCoins</b>\n"
        f"🏆 <b>Побед: {wins}</b> | 📉 <b>Поражений: {losses}</b>\n"
        f"📊 <b>Винрейт: {winrate:.1f}%</b> ({games} игр)\n"
        "━━━━━━━━━━━━━━━━━━━━━\n\n"
    )
    for level in range(1, 6):
        name, price = VIP_NAMES[level]['name'], VIP_PRICES[level]
        wb, db_bonus = VIP_NAMES[level]['win_bonus'], VIP_NAMES[level]['daily_bonus']
        text += f"{name} ({level} ур.) — {price} NCoins\n├ 🎮 +{wb}% к выигрышам\n└ 🎁 +{db_bonus} NCoins к бонусу\n\n"
    
    text += "━━━━━━━━━━━━━━━━━━━━━\n\n🎁 <b>БЕСПЛАТНЫЙ VIP ЗА ПОБЕДЫ В XO:</b>\n"
    for level in range(1, 5):
        text += f"├ {VIP_NAMES[level]['wins_required']} побед → {VIP_NAMES[level]['name']}\n"
    text += f"└ {VIP_NAMES[5]['wins_required']} побед → {VIP_NAMES[5]['name']}\n\n"
    text += f"📊 <b>Ваш прогресс: {wins} побед</b>"
    return text


def format_achievements(balance: int, wins: int, losses: int, games: int, current_vip: int, awarded_vip: Optional[int] = None) -> str:
    winrate = (wins / games * 100) if games > 0 else 0
    ...
    for level, required in thresholds:
        if wins < required:
            next_level, next_wins = VIP_NAMES[level]['name'], required
            prev = thresholds[level-2][1] if level > 1 else 0
            progress = max(0, min(100, int(((wins-prev)/(required-prev))*100)))
            break
    if next_level is None: progress = 100
    ...
    status_msg = ""
    if awarded_vip: status_msg = f"✨ <b>Только что получен VIP {awarded_vip} уровня!</b>\n\n"
    elif wins >= 500: status_msg = "🎉 <b>ВЫ ДОСТИГЛИ АЛМАЗА!</b>\n\n"
    elif wins >= 200: status_msg = "🎉 <b>ВЫ ДОСТИГЛИ ПЛАТИНЫ!</b>\n\n"
    elif wins >= 100: status_msg = "🎉 <b>ВЫ ДОСТИГЛИ ЗОЛОТА!</b>\n\n"
    elif wins >= 50: status_msg = "🎉 <b>ВЫ ДОСТИГЛИ СЕРЕБРА!</b>\n\n"
    elif wins >= 10: status_msg = "🎉 <b>ВЫ ДОСТИГЛИ БРОНЗЫ!</b>\n\n"
    ...
    if next_level and wins < 500:
        progress_msg = (
            ...
            f"[{progress_bar}] {progress}%\n\n💪 Осталось <b>{next_wins - wins}</b> побед!\n"
        )
    ...
    return (
        ...
        f"📊 Винрейт: <b>{winrate:.1f}%</b> ({games} игр)\n⭐ Текущий VIP: <b>{current_vip} уровень</b>\n\n"
...
async def cmd_vip(message: Message) -> None:
    ...
    if not user: await message.answer("❌ Ошибка доступа."); return
    ...
    try:
        ...
        if vip_level < 5:
            ...
            if awarded_vip: vip_level = awarded_vip; user = await db.get_user(user_id); balance = await db.get_balance(user_id)
        
        try: until_date = datetime.fromisoformat(vip_until).strftime("%d.%m.%Y") if vip_until else "Бессрочно"
        except: until_date = "Бессрочно"
        
        text = format_vip_active(vip_level, balance, wins, losses, until_date) if vip_level > 0 else format_vip_catalog(balance, wins, losses, games)
    ...
    except DatabaseError as e: logger.error(f"Database error: {e}"); await message.answer("❌ Ошибка загрузки VIP меню.")
    except Exception as e: logger.error(f"Unexpected error: {e}"); await message.answer("❌ Произошла ошибка.")
...
async def vip_callback(callback: CallbackQuery) -> None:
    ...
    await cmd_vip(callback.message); await callback.answer()
...
async def buy_vip_menu(callback: CallbackQuery) -> None:
    ...
    if not user: await callback.answer("❌ Ошибка БД", show_alert=True); return
    ...
    try:
        ...
        for level, price in VIP_PRICES.items():
            ...
            if level <= current_vip: name = f"✅ {name} (куплен)"
            buttons.append([InlineKeyboardButton(text=f"{name} ({price} NCoins)", callback_data=f"buy_vip_{level}")])
        buttons.append([InlineKeyboardButton(text="◀️ НАЗАД", callback_data="vip")])
        
        await callback.message.edit_text(
            f"💎 <b>ПОКУПКА VIP</b>\n\n💰 Баланс: <b>{balance} NCoins</b>\n⭐ Текущий VIP: <b>{current_vip} уровень</b>\n\nВыберите уровень:\n💡 VIP действует {VIP_DURATION_DAYS} дней",
            parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def buy_vip(callback: CallbackQuery) -> None:
    if not callback or not callback.message: return
    user_id = callback.from_user.id
    try: level = int(callback.data.split("_")[2])
    except: await callback.answer("❌ Неверный уровень!", show_alert=True); return
    ...
    current_vip = user.get('vip_level', 0) or 0
    if level <= current_vip: await callback.answer(f"❌ Уже есть VIP {current_vip} уровня!", show_alert=True); return
    
    try:
        balance = await db.get_balance(user_id)
        if balance < price:
            await callback.answer(f"❌ Недостаточно средств!\nНужно: {price} NCoins\nБаланс: {balance} NCoins", show_alert=True); return
        ...
        await callback.message.edit_text(
            ...
            f"💰 Списано: <b>{price} NCoins</b>\n💎 Новый баланс: <b>{new_balance} NCoins</b>\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n\n<b>✨ НОВЫЕ ПРЕИМУЩЕСТВА:</b>\n"
            f"├ 🎮 +{privileges['win_bonus']}% к выигрышам\n├ 🎁 +{privileges['daily_bonus']} NCoins к бонусу\n"
            f"├ 👑 Статус в чате\n├ 💎 Доступ к VIP-комнатам\n└ ⭐ Приоритетная поддержка\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n\n📅 Статус действует <b>{VIP_DURATION_DAYS} дней</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✅ ПОНЯТНО", callback_data="vip")]])
        )
        logger.info(f"User {user_id} purchased VIP level {level}")
    except DatabaseError as e: logger.error(f"VIP purchase failed: {e}"); await callback.answer("❌ Ошибка при покупке", show_alert=True)
    except Exception as e: logger.error(f"Unexpected error: {e}"); await callback.answer("❌ Произошла ошибка", show_alert=True)
    await callback.answer()
...
async def vip_achievements(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        current_vip = user.get('vip_level', 0) or 0
        awarded_vip = await check_and_award_achievement_vip(user_id, wins)
        if awarded_vip: current_vip = awarded_vip; balance = await db.get_balance(user_id)
        
        text = format_achievements(balance, wins, losses, games, current_vip, awarded_vip)
        await callback.message.edit_text(text, parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="◀️ НАЗАД", callback_data="vip")]]))
        await callback.answer()
    except DatabaseError as e: logger.error(f"Database error: {e}"); await callback.answer("❌ Ошибка загрузки", show_alert=True)
    except Exception as e: logger.error(f"Unexpected error: {e}"); await callback.answer("❌ Произошла ошибка", show_alert=True)
